               ...
               (let ((type `(simple-array (,(if signedp 'signed-byte 'unsigned-byte) ,width) (*)))
                 ...
                 `(progn
                    ...
                    (c::define-transform (setf sse-vector-ref) ((sse-vector sse-vector) (vector ,type array-type) (n-lanes (eql ,n-lanes)) (index fixnum index-type))
                      ...
                      `(the sse-vector (progn
                                         ,(c::insert-bounds-check vector array-type index index-type :adjust (1- ,n-lanes))
                                         (c::call (setf ,',access-fn) ,sse-vector ,vector (c::call c::%fast-fixnum-* ,index ',',(/ width 8))))))))))
    (def 8 nil)
    (def 16 nil)
    (def 32 nil)
    (def 64 nil)
...
(defconstant +u8.16-zero+
  (make-sse-vector 0))

(defconstant +u8.16-all-ones+
  (make-sse-vector #xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))

(defconstant +u32.4-low-byte-mask+
  (make-sse-vector #x000000FF000000FF000000FF000000FF))

(declaim (inline u8.16*-long))
(defun u8.16*-long (lhs rhs)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type sse-vector lhs rhs))
  (pmullw (punpcklbw lhs +u8.16-zero+)
          (punpcklbw rhs +u8.16-zero+)))

(declaim (inline u8.16*-long-hi))
(defun u8.16*-long-hi (lhs rhs)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type sse-vector lhs rhs))
  (pmullw (punpckhbw lhs +u8.16-zero+)
          (punpckhbw rhs +u8.16-zero+)))

(declaim (inline u8.16-not))
(defun u8.16-not (value)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type sse-vector value))
  (pxor value +u8.16-all-ones+))

(declaim (inline u8.16-dup))
(defun u8.16-dup (value lane)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type sse-vector value)
           (type (integer 0 15) lane))
  (let* ((byte (ldb (byte 8 (* lane 8)) (sse-vector-value value)))
         (splat (* byte #x01010101010101010101010101010101)))
    (make-sse-vector splat)))

(declaim (inline %extract-u32-channel-as-u8.16))
(defun %extract-u32-channel-as-u8.16 (value shift)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type sse-vector value)
           (type (integer 0 31) shift))
  (let* ((shifted (if (zerop shift)
                      value
                      (psrld value shift)))
         (masked (pand shifted +u32.4-low-byte-mask+))
         (words (packssdw masked +u8.16-zero+)))
    (packuswb words +u8.16-zero+)))

(declaim (inline %join-u8.16-channel-blocks))
(defun %join-u8.16-channel-blocks (x0 x1 x2 x3)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type sse-vector x0 x1 x2 x3))
  (let ((x01 (punpckldq x0 x1))
        (x23 (punpckldq x2 x3)))
    (punpcklqdq x01 x23)))

(declaim (inline u8.16-aref-4-interleaved-in-u32))
(defun u8.16-aref-4-interleaved-in-u32 (array &optional (index 0))
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type (simple-array (unsigned-byte 32) (*)) array)
           (type fixnum index))
  (let* ((x0 (u32.4-aref array index))
         (x1 (u32.4-aref array (the fixnum (+ index 4))))
         (x2 (u32.4-aref array (the fixnum (+ index 8))))
         (x3 (u32.4-aref array (the fixnum (+ index 12)))))
    (declare (type sse-vector x0 x1 x2 x3))
    (values (%join-u8.16-channel-blocks
             (%extract-u32-channel-as-u8.16 x0 0)
             (%extract-u32-channel-as-u8.16 x1 0)
             (%extract-u32-channel-as-u8.16 x2 0)
             (%extract-u32-channel-as-u8.16 x3 0))
            (%join-u8.16-channel-blocks
             (%extract-u32-channel-as-u8.16 x0 8)
             (%extract-u32-channel-as-u8.16 x1 8)
             (%extract-u32-channel-as-u8.16 x2 8)
             (%extract-u32-channel-as-u8.16 x3 8))
            (%join-u8.16-channel-blocks
             (%extract-u32-channel-as-u8.16 x0 16)
             (%extract-u32-channel-as-u8.16 x1 16)
             (%extract-u32-channel-as-u8.16 x2 16)
             (%extract-u32-channel-as-u8.16 x3 16))
            (%join-u8.16-channel-blocks
             (%extract-u32-channel-as-u8.16 x0 24)
             (%extract-u32-channel-as-u8.16 x1 24)
             (%extract-u32-channel-as-u8.16 x2 24)
             (%extract-u32-channel-as-u8.16 x3 24)))))

(declaim (inline set-u8.16-aref-4-interleaved-in-u32))
(defun set-u8.16-aref-4-interleaved-in-u32 (array index bb bg br ba)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type (simple-array (unsigned-byte 32) (*)) array)
           (type fixnum index)
           (type sse-vector bb bg br ba))
  (let* ((bg-lo (punpcklbw bb bg))
         (bg-hi (punpckhbw bb bg))
         (ra-lo (punpcklbw br ba))
         (ra-hi (punpckhbw br ba))
         (x0 (punpcklwd bg-lo ra-lo))
         (x1 (punpckhwd bg-lo ra-lo))
         (x2 (punpcklwd bg-hi ra-hi))
         (x3 (punpckhwd bg-hi ra-hi)))
    (declare (type sse-vector bg-lo bg-hi ra-lo ra-hi x0 x1 x2 x3))
    (setf (u32.4-aref array index) x0
          (u32.4-aref array (the fixnum (+ index 4))) x1
          (u32.4-aref array (the fixnum (+ index 8))) x2
          (u32.4-aref array (the fixnum (+ index 12))) x3)))

(eval-when (:compile-toplevel :load-toplevel :execute)
  (export 'u8.16*-long)
  (export 'u8.16*-long-hi)
  (export 'u8.16-not)
  (export 'u8.16-dup)
  (export 'u8.16-aref-4-interleaved-in-u32)
  (export 'set-u8.16-aref-4-interleaved-in-u32)
  (c::mark-as-constant-foldable 'u8.16*-long)
  (c::mark-as-constant-foldable 'u8.16*-long-hi)
  (c::mark-as-constant-foldable 'u8.16-not)
  (c::mark-as-constant-foldable 'u8.16-dup)
  (c::mark-as-constant-foldable '%extract-u32-channel-as-u8.16)
  (c::mark-as-constant-foldable '%join-u8.16-channel-blocks))
